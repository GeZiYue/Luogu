...
int main() {
  ...
  for (int j = 1; j <= lim; ++j) {
    for (int i = 1; i <= n; ++i) {
      q[i] = Point(i - 1, s[i] - dp[i][j - 1]);
    }
    que[qh = qt = 1] = 1;
    for (int i = 2; i <= n; ++i) {
      Point tmp(i, s[i]);
      while (qh < qt && slope(q[que[qh]], tmp) < slope(q[que[qh + 1]], tmp)) {
        ++qh;
      }
      dp[i][j] = (dp[que[qh]][j - 1] + s[i] - s[que[qh]]) / (i - que[qh] + 1);
      tran[i][j] = que[qh];
      while (qh < qt && slope(q[que[qt - 1]], q[que[qt]]) > slope(q[que[qt]], q[i])) {
        --qt;
      }
      que[++qt] = i;
    }
...
double slope(Point i, Point j) { return (j.y - i.y) / (j.x - i.x); }

Decimal calc(int i, int j) {
  if (!j) {
    return h[1];
  }
  return (s[i] - s[tran[i][j]] + calc(tran[i][j], j - 1)) / (i - tran[i][j] + 1);
}
